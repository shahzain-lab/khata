...
module.exports = composePlugins(
	...
	(config) => {
		...
		// Add CopyWebpackPlugin with the generated patterns (skip when none — empty patterns fail schema)
		if (packagePatterns.length > 0) {
			config.plugins.push(new CopyWebpackPlugin({ patterns: packagePatterns }));
		} else {
			console.warn('   No built packages found under dist/packages — run yarn build:package:all first');
		}
