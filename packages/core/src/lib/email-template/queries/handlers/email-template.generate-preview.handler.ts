...
export class EmailTemplateGeneratePreviewHandler implements IQueryHandler<EmailTemplateGeneratePreviewQuery> {
	...
	public async execute(command: EmailTemplateGeneratePreviewQuery): Promise<{ html: string }> {
		...
		const html = handlebarsTemplate({
			...
			agenda: 'This booking is for Khata call',
			...
			log_type: 'tracked',
			projects: ['Khata Website', 'Khata'],
			project: 'Khata Website',
			...
			task_update_project: 'Khata Project',
			task_update_assign_by: 'Ruslan Konviser',
			task_update_url: 'https://github.com/ever-co/ever-Khata/issues/1688',
			...
			items: [
				{
					tenantName: 'Default',
					userName: 'Default',
					resetLink: 'https://github.com/ever-co/ever-Khata'
				}
			],
			companyLink,
			companyName
