...
export const createDefaultReport = async (
	...
	// Define default categories (Khata: financial only)
	const defaultCategories: IReportCategory[] = [
		new ReportCategory({ name: 'Payments', iconClass: 'fa-credit-card' }),
		new ReportCategory({ name: 'Expenses', iconClass: 'fa-minus-circle' }),
	...
	// Define default reports (accounting / financial only)
	const reports: IReport[] = [
		new Report({
			name: 'Expense',
			slug: 'expense',
			image: copyAssets('expense.png', config, 'reports'),
			category: categoryByName['Expenses'],
			iconClass: 'far fa-credit-card',
			showInMenu: true,
			description: 'See how much has been spent on expenses.'
		}),
		new Report({
			name: 'Amounts owed',
			slug: 'amounts-owed',
			...
			iconClass: 'far fa-credit-card',
			showInMenu: true,
			description: 'See outstanding amounts owed'
		}),
		new Report({
			name: 'Payments',
			slug: 'payments',
			...
			iconClass: 'far fa-credit-card',
			showInMenu: true,
			description: 'See payments over a given period'
		}),
		new Report({
			name: 'Client budgets',
			slug: 'client-budgets',
