...
export class BaseNavMenuComponent extends TranslationBaseComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private _getMainMenu(): NavMenuSectionItem[] {
		// Khata: accounting + reports only (plus billing clients under accounting)
		return [
		...
		return [
			{
				...
				...(this._customDashboards.length === 0
					? { link: '/pages/dashboard/accounting' }
	 ...
	 * the "Standard" (accounting) dashboard followed by each custom dashboard.
	 */
