...
export const customDashboardGuard: CanActivateFn = async (route: ActivatedRouteSnapshot) => {
	...
	if (!id) {
		return router.parseUrl(`${DASHBOARD_BASE}/accounting`);
	}
...
/**
 * Guard for the standard dashboard tab routes (accounting, etc.).
 *
 ...
 * otherwise to the accounting dashboard.
 */
export const defaultDashboardGuard: CanActivateFn = async () => {
	...
	try {
		...
		return dashboard
			...
			: router.parseUrl(`${DASHBOARD_BASE}/accounting`);
	} catch (error) {
